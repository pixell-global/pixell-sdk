...
class ExecutionEngine:
    ...
    async def execute_stream(
        ...
        try:
            ...
            async for chunk in self.container_executor.execute_stream(
                plan.code, exec_context.full_context, plan.resource_tier, plan.timeout
            ):
                yield chunk
    ...
    async def understand(
        self, code: str, context: Dict[str, Any], files: List[bytes], session: Session
    ) -> ExecutionContext:
    ...
    async def execute(
        self, plan: ExecutionPlan, context: ExecutionContext, session: Session
    ) -> ExecutionResult:
        """Execute phase: run code in container."""
        return await self.container_executor.execute(
            plan.code, context.full_context, plan.resource_tier, plan.timeout
        )

    async def evaluate(
        self, result: ExecutionResult, plan: ExecutionPlan, attempt: int
        ...
        for key, value in context.items():
            if hasattr(value, "shape"):  # numpy/pandas
                data_info[key] = {
                    ...
                    "size": getattr(
                        value, "size", len(value) if hasattr(value, "__len__") else None
                    ),
                }
    ...
    def _estimate_resource_tier(self, context: ExecutionContext) -> int:
        ...
        # Check data sizes
        total_size = sum(info.get("size", 0) for info in context.data_info.values())
    ...
    def _optimize_code(self, code: str, patterns: List[str]) -> str:
        ...
        if "pandas" in patterns:
            ...
            optimized = re.sub(r"(\w+)\[([^\]]+)\]\[([^\]]+)\]", r"\1.loc[\2, \3]", optimized)
