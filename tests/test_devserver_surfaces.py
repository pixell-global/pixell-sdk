...
""")
    ...
    def test_dev_server_serves_ui_surface(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mounts = [mount.path for mount in server.app.routes if hasattr(mount, 'path')]
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('builtins.print') as mock_print:
                ...
                warning_calls = [call for call in mock_print.call_args_list 
                               if 'REST mount failed' in str(call)]
    ...
    def test_dev_server_handles_missing_ui_path(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('builtins.print') as mock_print:
                ...
                mock_print.assert_called()
                warning_calls = [call for call in mock_print.call_args_list 
                               if 'UI path not found' in str(call)]
                assert len(warning_calls) > 0
...
""")
            ...
            # Create UI directory
            (project_dir / "ui").mkdir()
            (project_dir / "ui" / "index.html").write_text("<html><body>Full Agent UI</body></html>")
...
""")
            ...
            with patch('builtins.print') as mock_print:
                ...
                print_calls = [str(call) for call in mock_print.call_args_list]
                surface_messages = [call for call in print_calls 
